...
app.get("/phone/:num", function (request, response) {
  ...
  if (phone === request.params.num) {
    response.send({
      status: 'ok',
      message:
        "Вам на номер " + request.params.num + " отправлен код подтверждения.",
    });
  } else {
    response.send({
      status: 404,
...
app.post("/registration", function (request, response) {
  ...
  response.send({
    status: "ok",
    message:
      "Вам на Email " + request.body.email + " отправлен код подтверждения.",
  });
});
...
const updateData = {
  ...
  companies: { storageName: "COMPANIES", ts: dateNow, info: companies },
};
...
app.post("/edit_user/:id", function (request, response) {
  if (!request.body) return response.sendStatus(400);
  console.log(request.body);
...
app.get("/check_update/:id", function (request, response) {
  if (!timeID) {
    generateTs();
  }
  let newArr = [];
...
app.get("/companies/:id", function (request, response) {
  ...
  const company = updateData.companies.info.filter(
    (company) => request.params.id === company.id
  );
...
app.get("/companies", function (request, response) {
  response.json({ data: updateData.companies });
});

app.get("/get_data/:ts/:id", function (request, response) {
